...
function CodePanel({
  ...
  let child = childArray.length === 1 ? childArray[0] : null

  if (child && isValidElement(child)) {
    const childProps = child.props as { tag?: string; label?: string; code?: string }
    tag = childProps.tag ?? tag
    label = childProps.label ?? label
    code = childProps.code ?? code
  }

  // If code is still not available, try to extract from children
  if (!code) {
  ...
  return (
    <div className="group dark:bg-white/2.5">
      ...
      <div className="relative">
        <pre className="overflow-x-auto p-4 text-xs text-white leading-6 [&>code]:block">{children}</pre>
