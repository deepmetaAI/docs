export const metadata = {
  title: 'OpenAI 图像生成',
  description: 'Cloubic OpenAI 标准协议图像生成接口，支持 GPT Image 等模型。',
}

# OpenAI 图像生成

Cloubic 目前已支持 OpenAI 标准协议的图像生成、编辑等操作的统一接入与调用。通过本章节提供的接口说明，您可以在同一套 API 下发起图像生成请求，屏蔽底层厂商差异，实现参数格式的统一管理与灵活扩展，完成通过文本对话生成图像的场景接入。{{ className: 'lead' }}
...
<Properties>
  ...
  <Property name="gpt-image-2" type="model">
    GPT Image 2 是 OpenAI 的图像生成模型，支持更高质量的图像生成与编辑能力。
  </Property>
  ...
  <Col>
    ...
    <Properties>
      <Property name="model" type="string" required>
        创建图片时选择的模型 ID，如 `gpt-image-1`、`gpt-image-2` 或 `dall-e-3`
      </Property>
      ...
      <Property name="quality" type="string">
        图像质量与细节偏好，不同模型支持的取值不同：

        - `gpt-image-1`、`gpt-image-2`：可选 `low`、`medium`、`high`，默认通常为 `medium`
        - `dall-e-3`：可选 `standard` 或 `hd`
      </Property>
...
<Row>
  <Col>
    <Endpoint method="POST" path="https://api.cloubic.com/v1/images/edits" />
    ...
    <Properties>
      <Property name="model" type="string" required>
        创建图片时选择的模型 ID，如 `gpt-image-1` 或 `gpt-image-2`
      </Property>
      <Property name="prompt" type="string" required>
        期望生成图片的文本描述
      </Property>
      ...
      <Property name="quality" type="string">
        图像质量与细节偏好，适用于 `gpt-image-1`、`gpt-image-2`。可选 `low`、`medium`、`high`，默认通常为 `medium`。
      </Property>
      <Property name="size" type="string">
        生成图片的尺寸，默认 `1024x1024`
      </Property>
...
```bash {{ title: 'cURL' }}
curl https://api.cloubic.com/v1/images/edits \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -F "model=gpt-image-1" \
