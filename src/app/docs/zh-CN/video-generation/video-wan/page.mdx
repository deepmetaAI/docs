export const metadata = {
  title: 'Wan 视频生成',
  description: 'Cloubic Wan（万相）视频生成接口，支持文生视频、图生视频和图生动作。',
}

# Wan 视频生成

Cloubic 目前已支持对 Wan 视频大模型的统一接入与调用。通过本章节提供的接口说明，可完成对万相文生视频、图生视频、图生动作等模型调用。{{ className: 'lead' }}
...
<Properties>
  ...
  <Property name="wan2.2-animate-move" type="model">
    图生动作（将参考视频中的动作/表情迁移到图片角色）
  </Property>
</Properties>
...
<Row>
  <Col>
    <Endpoint method="POST" path="https://api.cloubic.com/v1/videos" />
    ...
    <Properties>
      ...
      <Property name="size" type="string">
        ...
        <Options>
          ...
          <Option value="1280*720" label="16:9 (720P)" />
          <Option value="720*1280" label="9:16 (720P)" />
          <Option value="960*960" label="1:1 (720P)" />
          <Option value="1088*832" label="4:3 (720P)" />
          <Option value="832*1088" label="3:4 (720P)" />
        </Options>
      </Property>
...
```bash {{ title: 'cURL' }}
curl https://api.cloubic.com/v1/videos \
...
url = 'https://api.cloubic.com/v1/videos'
...
基于人物图片和参考视频，将视频中的动作和表情迁移到图片角色中，生成动作视频。适用于复刻舞蹈、高难度肢体动作、影视剧表演等场景，可作为低成本动捕替代方案。
...
<Row>
  <Col>
    ...
    <Properties>
      <Property name="model" type="string" required>
        模型 ID，固定为 `wan2.2-animate-move`
      </Property>
      <Property name="image_url" type="string" required>
        人物图片的公网可访问 URL
      </Property>
      <Property name="video_url" type="string" required>
        参考视频的公网可访问 URL
      </Property>
      <Property name="mode" type="string" required>
        服务模式。`wan-std`：标准模式；`wan-pro`：专业模式
      </Property>
    </Properties>
  </Col>
  <Col sticky>
...
<Row>
  <Col>
    <Endpoint method="GET" path="https://api.cloubic.com/v1/videos/{video_id}" />
...
```bash {{ title: 'cURL' }}
curl 'https://api.cloubic.com/v1/videos/{video_id}' \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
<Row>
  <Col>
    <Endpoint method="GET" path="https://api.cloubic.com/v1/videos/{video_id}/content" />
...
```bash {{ title: 'cURL' }}
curl 'https://api.cloubic.com/v1/videos/{video_id}/content' \
  -H "Authorization: Bearer YOUR_API_KEY" \
  --output video.mp4
```
