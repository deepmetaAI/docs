export const metadata = {
  title: 'Jimeng 视频生成',
  description: 'Cloubic Jimeng（即梦）视频生成接口，支持文生视频和图生视频。',
}

# Jimeng（即梦）视频生成

Cloubic 目前已支持对 Jimeng（即梦）视频大模型的统一接入与调用。通过本章节提供的接口说明，您可以在同一套 API 下发起视频生成请求，调用 Jimeng 视频生成接口生成视频。{{ className: 'lead' }}
...
const url = 'https://api.omnimaas.cn/v1/video/generations';
...
```bash {{ title: 'cURL' }}
curl https://api.omnimaas.cn/v1/video/generations/{task_id} \
  -H "Authorization: Bearer YOUR_API_KEY"
```
