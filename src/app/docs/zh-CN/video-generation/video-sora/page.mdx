export const metadata = {
  title: 'Sora 视频生成',
  description: 'Cloubic Sora 视频生成接口，支持文生视频和图生视频。',
}

# OpenAI 视频生成（Sora）

Cloubic 目前已支持对 Sora、Veo 视频大模型的统一接入与调用。通过本章节提供的接口说明，您可以在同一套 API 下发起视频生成请求，调用 OpenAI 视频生成接口生成视频，支持 Sora 等模型。{{ className: 'lead' }}
...
func main() {
    ...
    req, _ := http.NewRequest("POST",
        "https://api.cloubic.com/v1/videos",
...
const url = 'https://api.cloubic.com/v1/videos';
...
```bash {{ title: 'cURL' }}
curl 'https://api.cloubic.com/v1/videos/video_123' \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash {{ title: 'cURL' }}
curl 'https://api.cloubic.com/v1/videos/video_123/content' \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -o "video.mp4"
```
